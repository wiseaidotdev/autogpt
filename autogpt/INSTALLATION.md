...
docker run -it \
  ...
  --rm --name autogpt kevinrsdev/autogpt:0.2.5 man
```
...
docker run -it \
  ...
  -e PINECONE_API_KEY=<Your_Pinecone_API_Key> \
  -e PINECONE_INDEX_URL=<Your_Pinecone_Index_URL> \
  --rm --name orchgpt kevinrsdev/orchgpt:0.2.5
```
...
docker run -i \
  ...
  -e PINECONE_API_KEY=<Your_Pinecone_API_Key> \
  -e PINECONE_INDEX_URL=<Your_Pinecone_Index_URL> \
  --rm --name autogpt kevinrsdev/autogpt:0.2.5
```
